...
for idx, num_comm_rounds in enumerate(comm_rounds_list):
    for coeff in lcp_coeff:
        ...
        axs[idx].plot(avg_l1, label=f"$\\alpha={coeff}$")   # use coeff in label
...
fig.savefig("finite_subgrid_comms_lcp_l1_error_vs_time_100_benchmark.png", dpi=300) 
